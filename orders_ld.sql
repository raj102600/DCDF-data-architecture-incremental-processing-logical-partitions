...
*/
CREATE or replace TABLE orders_ld (
    dw_order_shk VARCHAR(40),
...
begin
  ...
  for record in c1 do
    ...
    -- Merge Pattern 
    --
    merge into orders_ld tgt using
    (
        with l_stg as
        (
            --
            ...
            select
                -- generate hash key and hash diff to streamline processing
                 HEX_ENCODE(sha1_binary( s.o_orderkey ))  as dw_order_shk
                ...
                ,HEX_ENCODE(sha1_binary( concat( s.o_orderkey
